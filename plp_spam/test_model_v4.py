...
def save_log():
    ...
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        f.write("# YouTube Comment Analysis Log (SVM Model)\n\n")
...
def load_model_and_vectorizer():
    ...
    try:
        # Load model
        with open('spam_model_v4.pkl', 'rb') as f:
            model = pickle.load(f)
        
        # Load vectorizer
        with open('vectorizer_v4.pkl', 'rb') as f:
            vectorizer = pickle.load(f)
        
        log_message("Model and vectorizer loaded successfully")
        return model, vectorizer
    except FileNotFoundError:
        log_message("Error: Model files not found. Please run train_model_v4.py first to train the model.")
        return None, None
    except Exception as e:
...
def main():
    ...
    if not os.path.exists('spam_model_v4.pkl') or not os.path.exists('vectorizer_v4.pkl'):
        log_message("Error: Model files not found. Please run train_model_v4.py first to train the model.")
        save_log()
        return
    ...
    output_csv_path = 'comments_analysis_v4.csv'
