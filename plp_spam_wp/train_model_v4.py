...
def save_log():
    ...
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        f.write("# YouTube Spam Detection Model Training Log (SVM)\n\n")
...
# Train model
log_message("Training SVM (LinearSVC) model...")
model = LinearSVC(C=1.0, max_iter=10000, dual=False)
model.fit(xtrain, ytrain)

# Cross-validation
log_message("Performing 5-fold cross-validation...")
cv_scores = cross_val_score(LinearSVC(C=1.0, max_iter=10000, dual=False), x, y, cv=5)
...
with open('spam_model_v4.pkl', 'wb') as f:
    pickle.dump(model, f)
    
with open('vectorizer_v4.pkl', 'wb') as f:
    pickle.dump(cv, f)

log_message("Model and vectorizer saved to 'spam_model_v4.pkl' and 'vectorizer_v4.pkl'")
