...
try:
    ...
    # Ensure all punkt-related resources are downloaded
    nltk.download('punkt')
except LookupError:
...
def save_log():
    ...
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        f.write("# YouTube Spam Detection Model Training Log\n\n")
...
# Train model
log_message("Training BernoulliNB model...")
model = BernoulliNB()
model.fit(xtrain, ytrain)

# Cross-validation
log_message("Performing 5-fold cross-validation...")
cv_scores = cross_val_score(BernoulliNB(), x, y, cv=5)
...
with open('spam_model_v1.pkl', 'wb') as f:
    pickle.dump(model, f)
    
with open('vectorizer_v1.pkl', 'wb') as f:
    pickle.dump(cv, f)

log_message("Model and vectorizer saved to 'spam_model_v1.pkl' and 'vectorizer_v1.pkl'")
